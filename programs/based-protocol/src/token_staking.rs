...
pub struct DepositToken<'info> {
    #[account(mut, seeds = [b"token_pool", token_mint.key().as_ref()], bump = token_pool.bump)]
    pub token_pool: Account<'info, TokenStakingPool>,
    pub token_mint: Account<'info, Mint>,
    #[account(
        init_if_needed,
        payer = user,
        space = 8 + 32 + 8 + 8 + 8 + 8 + 8 + 33 + 8 + 8 + 8 + 8 + 8 + 1,
        seeds = [b"token_user_stake", token_pool.key().as_ref(), user.key().as_ref()],
        bump,
    )]
    ...
    #[account(
        init_if_needed,
        payer = user,
        seeds = [b"token_pool_vault", token_pool.key().as_ref()],
        bump,
        token::mint = token_mint,
        token::authority = token_pool,
    )]
    pub pool_token_account: Account<'info, TokenAccount>,
    #[account(mut, constraint = user_token_account.mint == token_pool.token_mint)]
...
pub struct WithdrawToken<'info> {
    ...
    pub token_pool: Account<'info, TokenStakingPool>,
    #[account(mut, seeds = [b"token_user_stake", token_pool.key().as_ref(), user.key().as_ref()], bump = token_user_stake.bump)]
    pub token_user_stake: Account<'info, TokenUserStake>,
    #[account(mut, seeds = [b"token_pool_vault", token_pool.key().as_ref()], bump)]
    pub pool_token_account: Account<'info, TokenAccount>,
    #[account(mut)]
    pub user_token_account: Account<'info, TokenAccount>,
    #[account(mut)]
